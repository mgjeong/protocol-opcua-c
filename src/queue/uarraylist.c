...
{
    u_arraylist_t *list = (u_arraylist_t *) EdgeCalloc(1, sizeof(u_arraylist_t));
    if (!list)
    {
        EDGE_LOG(TAG, "Out of memory");
        return NULL;
    }
    ...
    list->data = (void **) EdgeMalloc(list->capacity * sizeof(list->data[0]));
    if (!list->data)
    {
        EDGE_LOG(TAG, "Out of memory");
        EdgeFree(list);
...
bool u_arraylist_reserve(u_arraylist_t *list, size_t count)
{
    ...
    {
        void *tmp = EdgeRealloc(list->data, count * sizeof(list->data[0]));
        if (!tmp)
        {
            EDGE_LOG(TAG, "Memory reallocation failed.");
            return false;
        }
        else
        {
...
void u_arraylist_shrink_to_fit(u_arraylist_t *list)
{
    ...
    if ((list->capacity > list->length) && (list->length >= U_ARRAYLIST_DEFAULT_CAPACITY))
    {
        void *tmp = EdgeRealloc(list->data, list->length * sizeof(list->data[0]));
        if (!tmp)
        {
            EDGE_LOG(TAG, "Memory reallocation failed.");
...
bool u_arraylist_add(u_arraylist_t *list, void *data)
{
    ...
    if (list->capacity <= list->length)
    {
        ...
        void *tmp = EdgeRealloc(list->data, new_capacity * sizeof(list->data[0]));
        if (!tmp)
        {
            EDGE_LOG(TAG, "Memory reallocation failed.");
            return false;
...
uint32_t u_arraylist_length(const u_arraylist_t *list)
{
    if (!list)
    {
        EDGE_LOG(TAG, "Invalid Parameter");
        return 0;
    }
    return list->length;
}
